/**
 * Created by valeriu.jecov on 5/26/2017.
 */
...
describe(harness.getCommandLineArgs().appType + ':' + harness.getCommandLineArgs().appSubType +
    ...
    describe('open map from url', () => {
        it('open direct link', async () => {
            await driver.get(configJson.gisBasePublicUrl + '/UofC/Public/index.html');
	        await UofC.waitForObjectLoad('.modal-container.terms-of-use', waitLong * 5, waitLong);
        });

        it('click i accept button', async () => {
            await UofC.clickElementByCSS('*[value="I Accept"]');
        });

        it('opens expected url', async () => {
            const currentUrl = await driver.getCurrentUrl();
            const expectedUrl = configJson.gisBasePublicUrl + '/UofC/Public/index.html';
