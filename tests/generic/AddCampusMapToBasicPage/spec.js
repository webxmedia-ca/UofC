/**
 * Created by valeriu.jecov on 31/05/2019.
 * NOTE: THIS TEST IS NOT DONE
 * MANUAL TEST: UCWS-750 | https://ucalgary.atlassian.net/browse/UCWS-750
 */
...
describe('appName: ' + harness.getCommandLineArgs().appName + ' (user: ' + harness.getCommandLineArgs().role +
 ') | env: ' + harness.getCommandLineArgs().env + ' | BrowserStack: ' + harness.getCommandLineArgs().browserStack, function () {
	...
	const newCampusMapBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().addCampusMapBlock;
	UofC.addNewBlock(newCampusMapBlockValues);
	...
	const campusMapBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().editCampusMap;
