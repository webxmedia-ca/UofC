...
describe('appName: ' + harness.getCommandLineArgs().appName + ' (user: ' + harness.getCommandLineArgs().role +
 ') | env: ' + harness.getCommandLineArgs().env + ' | BrowserStack: ' + harness.getCommandLineArgs().browserStack, function () {
	...
	const newTextBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().addTextBlock;
	UofC.addNewBlock(newTextBlockValues);
	...
	const textBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().editText;
