...
describe('appName: ' + harness.getCommandLineArgs().appName + ' (user: ' + harness.getCommandLineArgs().role +
 ') | env: ' + harness.getCommandLineArgs().env + ' | BrowserStack: ' + harness.getCommandLineArgs().browserStack, function () {
	...
	const newRelatedContentBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().addRelatedContentBlock;
	UofC.addNewBlock(newRelatedContentBlockValues);
	
	describe('save page layout', () => {
		UofC.clickLayoutActionButtons('save', 'Close Status Message\nStatus message\nThe layout override has been saved.');
	});
	
	const relatedContentBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().editRelatedContent;
	const imageAttachmentFolderPath = require('path').join(__dirname, '/attachments/'); //this works differently than all other places
	UofC.editRelatedContentBlock(relatedContentBlockValues, imageAttachmentFolderPath);
});