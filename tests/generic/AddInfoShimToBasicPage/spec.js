...
describe('appName: ' + harness.getCommandLineArgs().appName + ' (user: ' + harness.getCommandLineArgs().role +
 ') | env: ' + harness.getCommandLineArgs().env + ' | BrowserStack: ' + harness.getCommandLineArgs().browserStack, function () {
	...
	const newInfoShimBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().addInfoShimBlock;
	UofC.addNewBlock(newInfoShimBlockValues);
	...
	const infoShimBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().editInfoShim;
