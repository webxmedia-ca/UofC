...
describe('appName: ' + harness.getCommandLineArgs().appName + ' (user: ' + harness.getCommandLineArgs().role +
 ') | env: ' + harness.getCommandLineArgs().env + ' | BrowserStack: ' + harness.getCommandLineArgs().browserStack, function () {
	...
	const newTestimonialBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().addTestimonialBlock;
	UofC.addNewBlock(newTestimonialBlockValues);
	...
	const testimonialBlockValues = new HarnessJson(dataJsonFilePath).getJsonData().editTestimonial;
	///// TODO: create the "editTestimonialBlock" functions
	UofC.editTestimonialBlock(testimonialBlockValues);  //-- this is not there yet - needs to be implemented
});