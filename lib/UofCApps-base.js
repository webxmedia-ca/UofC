...
const UofC = (() => {
	...
	UofC.afterEachTest = async (test) => {
		if (failureFatal) {
			const browserStack = harnessObj.getCommandLineArgs().browserStack;
			if (test.state === 'failed') {
				/*
				const retrieveBrowserConsoleLog = async () => {
					//some code..
				};
				await retrieveBrowserConsoleLog();
				*/
				
				console.log('\n*********************************************************************************************');
				console.log('------------------------------------ BROWSERSTACK SESSION ------------------------------------');
				//if test run through browserStack then send the fail to the test
				...
				//******************************************************************************************************
				// THIS PIECE HERE RETRIEVES BROWSER's CONSOLE LOG MESSAGES IF ANY EXIST AND SENDS THEM TO TEST's CONSOLE
				...
				if (getBrowser.length > 0) {
					...
					console.log('failureFatal = true & test.state = failed and there are no messages in the browser\'s console');
				}
				// console.log('\nEND: exiting after test failed');
				// console.log('----------------------------------------------------------------------------------------\n');
			} else {
				console.log('\n*********************************************************************************************');
				console.log('------------------------------------ BROWSERSTACK SESSION ------------------------------------');
				...
				if (browserStack === 'true') {
					...
					request({
						uri: "https://valeriujecov1:c8EHLyzv9zWTJ2zGRedY@api.browserstack.com/automate/sessions/sessionId.json",
						method: "PUT",
						form: {"status": "passed", "reason": "test successfully passed, test.state: " + test.state}
					});
				}
				// console.log('\nEND: exiting after test passed');
				// console.log('----------------------------------------------------------------------------------------\n');
			}
			console.log('\nEND: exiting after test ' + test.state);
			console.log('----------------------------------------------------------------------------------------\n');
	...
	UofC.login = async () => {
		...
		// if (harnessObj.getUserRole() === 'siteadmin') {
			...
			console.log('---> login: user successfully logged in as ' + harnessObj.getUserRole() + '\n\n');
			//LOGIN DONE
		// }
			...
			if (accordionItems.length > 0) {
				...
				for (let i = 0; i < 30; i++) {
					...
					if (accordionTitleItem === itemName) {
						...
						await driver.executeScript('arguments[0].scrollIntoView({block: "end", inline: "nearest"});', accordionItems[i]);
	...
	UofC.clickLinkFromGroupByName = (linksGroupLocator, linkNameToClick, expectedCssLocator) => {
		...
		/*it('click ' + linkNameToClick + ' link', async () => {
			...
			if (await !driver.findElements(By.css(linksGroupLocator)).length) {
				//get the links number from inside of the group locator
				const linkElementsCounter = await driver.findElements(By.css(linksGroupLocator + ' a:not([style="display: none;"])')).length;
				//validate there are any links inside the group locator (===> truthy)
	...
	UofC.waitForPageLoad = async () => {
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////
		/*
		//LAST ONE -- this works BUT - if the element is gone when it created the elStale then the loop never ends
		//NOT FULLY WORKING - thought the idea is good ----->>> to keep an eye on this method (stalenessOf)
		let spinnerElement = await driver.findElements(By.css('div.loading[style*=block]'));
		if (spinnerElement.length > 0) {
			//driver.wait(until.elementLocated(By.name('q')))
			console.log('before waitPage');
			let elStale = await driver.wait(until.stalenessOf(spinnerElement[0]), waitLong * 10);
			console.log('after waitPage, elStale: ' + elStale);
		} else {
	...
	UofC.popUpConfirmation = async (message, waitDelay) => {
		...
		----------------------------------------------------------------------------------------------------------------- */
		it('check the warning message is displayed with the "' + message + '" text', async () => {
			// await UofC.waitForPageLoad();
		...
		it('close the popup message dialog', async () => {
			...
			// await UofC.clickElementByCSS('.messages>button');
		});
	};
...
/* --------- to be modified / used across UofC projects -------- */
