		...
		it('click on ' + itemName + ' accordion menu to expand ' + expandItem, async () => {
			...
			if (accordionItems.length > 0) {
				...
				for (let i = 0; i < 30; i++) {
					...
					// if (accordionTitleItem) {    //not sure what is the point of this -- to later see app's behaviour
					...
					// } else if (i >= accordionItemsCounter && !accordionTitleItem) {
					// 	throw 'no accordion objects found on the page';
					// }
	...
	UofC.clickLinkFromGroupByName = (linksGroupLocator, linkNameToClick, expectedCssLocator) => {
		...
		it('click ' + linkNameToClick + ' link', async () => {
			...
			if (await !driver.findElements(By.css(linksGroupLocator)).length) {
				...
				const linkElementsCounter = driver.findElements(By.css(linksGroupLocator + ' a:not([style="display: none;"])')).length;
