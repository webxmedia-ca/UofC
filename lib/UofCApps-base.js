...
const UofC = (() => {
	...
	UofC.login = async (overrideUrl) => {
		...
		// if (harnessObj.getUserRole() === 'siteadmin') { //TODO: add those extra users (roles) which have the login fncts
			...
			console.log('successfully logged in as ' + harnessObj.getUserRole() + ' user\n');
			//LOGIN DONE
		// }
	...
	UofC.logout = async () => {
		...
		//TODO: uncomment the line below when the logout bug is fixed
	...
	UofC.waitForPageLoad = async () => {
		...
		if (elementsSpinner.length > 0) {
			...
			while (elementsSpinner.length > 0) {
				...
				// elementsSpinner = await driver.findElements(By.css('div.loading[style*=block]'));   //this not needed - maybe later
			}
			// } else {
