...
const UcLaw = (() => {
	...
	UcLaw.createBasicPage = (newPageParameters) => {
		describe('create a new basic page', () => {
			...
			describe('setup the new basic page and fill it wih data', () => {
				...
				if (newPageParameters.menuSettingsValues) {
					...
					// const menuWeight = newPageParameters.menuWeight;
	...
	UcLaw.addNewBlock = (addBlockBtnNr, blockCategory, blockExpectedCssLocator, categoryLinkGroupLocator,
	                     categoryLinkNameToClick, categoryExpectedCssLocator, checkBoxesToSelect,
		...
		describe('add a new block using btn nr 2', () => {
			...
			UcLaw.clickLinkFromGroupByName(categoryLinkGroupLocator, categoryLinkNameToClick,
			  categoryExpectedCssLocator);
			
			UcLaw.selectMultipleCheckboxesInAGroup('div[id*=edit-settings-teams]', checkBoxesToSelect);
