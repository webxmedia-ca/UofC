		...
		describe('edit the newly added block', () => {
			...
			if (mediaTypeEmbedUrl) {
				it('type the ' + mediaTypeValue.toLowerCase() + ' embed url as ' + mediaTypeEmbedUrl, async () => {
					let mediaTypeEmbedUrlLocator = (mediaTypeValue.toLowerCase() === 'youtube') ?
					  'input[id*=youem]' : 'input[id*=uri]';
					await UcLaw.clickElementByCSS(mediaTypeEmbedUrlLocator);
			...
			if (createNewRevision === 'true') {
				...
				it('type the revision log message as ' + revisionLogMessage, async () => {
					await UcLaw.setTextFieldValueByCSS('textarea[id*=edit-revision-log]', revisionLogMessage);
				});
			}
