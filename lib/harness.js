...
const testHarness = (function () {
	...
	const getCommandLineArgs = function () {
		...
		if (errMsg) {
			...
			// process.exit(1);
			throw '<<< ERROR: commandline args --- ' + errMsg + '>>>';//this line replaces both above ones
		}
	...
	*/
	...
	const init = async () => {
		...
		if(browserStack === 'true') {
			...
			if (harnessJsonObj.getJsonData().browserStack) {
				let capabilities = {
					...
					//these ones are taken from the command line (run npm... browserStackArgs)
				...
				throw '--browserStack argument is true but there are no "browserStack" parameters in the ./config/' +
				getEnv() + '.config.json file\n either\n the "browserStackArgs" commandline arguments are missing ' + browserStackArgs;
			}
		} else {
		...
		if (env !== 'all') {
			...
			//NOTE: this 'else' is temporary until I find the rules, might not be needed
			// 	baseUrl = configJson.baseUrl;
				if (appName === 'ucLaw') baseUrl = configJson.appsUrls.ucLaw;
				if (appName === 'ucWeb') baseUrl = configJson.appsUrls.ucWeb;
			}
