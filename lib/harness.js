			...
			for (let i = 0; i < suitesCounter; i++) {
				...
				if (testsCounter > 0) {
					for (let j = 0; j < testsCounter; j++) {
						//retrieve the title of the step
						testTitle = test.suites[i].tests[j].title;
						//retrieve test's state (failed / passed)
						testState = test.suites[i].tests[j].state;
						if(testState === 'failed') {
							//retrieve the error stack
							testErrorStack = test.suites[i].tests[j].err.stack;
