...
const testHarness = (function () {
	...
	*/
	...
	const init = async () => {
		...
		// if (getEnv() !== 'all') {    ///commenting this out as we don't have and 'all' env, at least not for now
			const harnessJsonObj = new HarnessJson('./config/' + getEnv() + '.config.json');
			configJson = harnessJsonObj.getJsonData();
		// }
		
		// load config details from json
		// if (env !== 'all') {         ///commenting this out as we don't have and 'all' env, at least not for now
